...
class ConversionJournalReportService extends BaseService implements ConversionJournalReportServiceInterface
{
    ...
    public function getAll(): LengthAwarePaginator
    {
        ...
        // Stock journal type filter (e.g. 'conv' for conversion journals)
        $stockJournalType = request()->input('stock_journal_type');
        if (! empty($stockJournalType)) {
            $query->whereHas('stock_journal', function ($q) use ($stockJournalType) {
                $q->where('type', $stockJournalType);
            });
        }
    ...
    public function getGroupedByStockItem(array $params = []): Collection
    {
        ...
        $query = DB::table('vouchers')
            ->where('vouchers.voucher_type_id', 2008)
        ...
        if (! empty($params['stock_journal_type'])) {
            $query->where('stock_journals.type', $params['stock_journal_type']);
        }
